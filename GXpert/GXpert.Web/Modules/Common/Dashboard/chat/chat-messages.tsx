...
const ChatMessage = ({ contact, time, children }: { contact: ChatContact, time: string, children: ReactNode }) => {
    ...
    return <div class={own ? "s-chat-message-own mb-4" : "s-chat-message-other pb-4"}>
        ...
        <div class={["s-chat-message-body flex-shrink-1 rounded py-2 px-3", own ? "me-3" : "ms-3"]}>
            {children}
        </div>
    </div >
}
...
export const ChatMessages = ({ messages }: { messages: ChatMessage[] }) =>
    <div class="s-chat-messages p-4">{messages.map(msg => <ChatMessage contact={msg.contact} time={msg.time}>{msg.message}</ChatMessage>)}</div>
