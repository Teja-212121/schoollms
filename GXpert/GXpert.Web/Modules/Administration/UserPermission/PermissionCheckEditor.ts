...
import {
    DataGrid, Decorators, Dictionary, GridUtils, Grouping, IGetEditValue, ISetEditValue, SlickFormatting, SlickTreeHelper, ToolButton,
    count, getRemoteDataAsync, htmlEncode, localText, toGrouping, tryGetText, turkishLocaleCompare
} from "@serenity-is/corelib";
import { Column } from "@serenity-is/sleekgrid";

@Decorators.registerEditor('GXpert.Administration.PermissionCheckEditor', [IGetEditValue, ISetEditValue])
export class PermissionCheckEditor extends DataGrid<PermissionCheckItem, PermissionCheckEditorOptions> {
    ...
    protected createToolbarExtensions(): void {
        ...
        GridUtils.addQuickSearchInputCustom(this.toolbar.element, (_, text) => {
            this.searchText = Select2.util.stripDiacritics(text?.trim() ?? '').toLowerCase();
