        ...
        IEmailSender emailSender, ISMSService smsService)
    {
             ...
             (user.TwoFactorAuth == TwoFactorAuthType.Email)))
        {
            ...
            if (user.TwoFactorAuth == TwoFactorAuthType.SMS)
            {
                ...
                smsService.Send(
                    phoneNumber: mobile,
                    text: "Please use code " + data.TwoFactorCode + " for GXpert login.",
                    reason: "Sent by GXpert system for two factor authenication by SMS (" + user.Username + ")");
            ...
            else
            {
                ...
                emailSender.Send(
                    mailTo: user.Email,
                    subject: "Your two-factor authentication code for GXpert login",
                    body: "Please use code " + data.TwoFactorCode + " for GXpert login.");
